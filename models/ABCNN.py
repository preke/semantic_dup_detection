...
class Abcnn1(nn.Module):
    def __init__(self, emb_dim, emb_num, sentence_length=25, 
                filter_width, filter_channel=100, layer_size=2, 
                inception=True, pretrained_weight=None, device):
        ...
        for i in range(layer_size):
            self.abcnn.append(Abcnn1Portion(sentence_length, emb_dim if i == 0 else filter_channel, self.device))
    ...
    def forward(self, batched_data):
        x1 = torch.tensor(batched_data[1]).to(self.device)
        x2 = torch.tensor(batched_data[2]).to(self.device)
        x1 = self.embed(x1)
        x1 = x1.unsqueeze(1)
...
class Abcnn1Portion(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        '''
        attention_m = attention_matrix(x1, x2, device)
...
class ConvLayer(nn.Module):
    ...
    def __init__(self, sentence_length, filter_width, filter_height, filter_channel, inception):
        ...
        if inception:
            self.model = InceptionModule(2, sentence_length, filter_width, filter_height, filter_channel)
        else:
            self.model = convolution(2, filter_width, filter_height, filter_channel, filter_width-1)
...
def attention_matrix(x1, x2, eps=1e-6, device):
